...
const authoritativeBaseSchema = `
  ...
  CREATE TABLE credit_ledger_entries (
    ...
    amount INTEGER, metadata_json TEXT, idempotency_scope TEXT,
  ...
  CREATE TABLE credit_purchase_transactions (
    ...
    payload_json TEXT, verified_at TEXT, updated_at TEXT
  );
...
test("deletion path verifies schema instead of creating route-local tables", () => {
  assert.match(
    routeSource,
    /if \(operation === "account_delete"\) \{\s*await assertAccountDeletionSchemaReady\(\);\s*\} else \{\s*await ensureAdminUserSchemas\(\);\s*\}/,
  );
  assert.doesNotMatch(
    routeSource,
    /if \(operation === "account_delete"\) \{\s*await ensureAdminUserSchemas\(\);/,
  );
});
