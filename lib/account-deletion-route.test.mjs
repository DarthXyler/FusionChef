...
async function createFixture() {
  ...
  await client.executeMultiple(`
    INSERT INTO auth_users (
      ...
      '${PUBLIC_BASE_URL}/profile-photos/auth-only-1780000000000-deadbeef.webp',
...
test("deletion route maps storage failures and logs only unexpected failures", () => {
  assert.match(
    routeSource,
    /const storageApiFailure = getAccountDeletionStorageApiFailure\(error\)/,
  );
  assert.match(routeSource, /storageApiFailure\.body\.error/);
  assert.match(routeSource, /storageApiFailure\.body\.code/);
  assert.match(routeSource, /storageApiFailure\?\.statusCode/);
  assert.match(
    routeSource,
    /!storageApiFailure[\s\S]*logUnexpectedAccountDeletionFailure/,
  );
});
...
test("auth-only preview executes through the durable job and replays idempotently", async () => {
  ...
  try {
    ...
    assert.deepEqual(graph.storageReferences, [
      {
        ...
        value: `${PUBLIC_BASE_URL}/profile-photos/auth-only-1780000000000-deadbeef.webp`,
      },
    ]);
    ...
    assert.deepEqual({ ...queued.rows[0] }, {
      object_key: "profile-photos/auth-only-1780000000000-deadbeef.webp",
...
test("auth-only preview becomes stale when its profile storage fact changes", async () => {
  ...
  try {
    ...
    await client.execute({
      ...
      args: [
        `${PUBLIC_BASE_URL}/profile-photos/changed-1780000000000-feedface.webp`,
        AUTH_ID,
      ],
    });
