    ...
    storageReferences: [
      {
        ...
        value: `https://cdn.example.test/recipe-images/${name}.webp`,
      },
      {
        category: "profile_avatar",
        value: `https://cdn.example.test/profile-photos/${name}.webp`,
      },
    ],
    inventory: inventory(),
  };
...
test("persisted plan hashes user, identity, device, and storage references", async () => {
  ...
  try {
    ...
    for (const raw of [
      ...
      "device-a",
      "https://cdn.example.test/recipe-images/a.webp",
      "https://cdn.example.test/profile-photos/a.webp",
    ]) {
...
test("later target failure preserves completed graph and retry skips it", async () => {
  ...
  try {
    ...
    assert.notEqual(retryGraphs[0], firstRunGraphs[0]);
