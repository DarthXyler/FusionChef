...
test("the normal admin UI uses account setup terminology", () => {
  ...
  assert.match(panelSource, /Setup incomplete/);
  assert.match(panelSource, /Shared app data/);
  assert.match(panelSource, /Split app data/);
  assert.match(panelSource, /Invalid account setup/);
  assert.match(panelSource, /getAdminUserIdentityIssueTooltip/);
  assert.match(panelSource, /min-w-\[13rem\]/);
  assert.match(panelSource, /bg-amber-100/);
  assert.match(panelSource, /text-zinc-500/);
  assert.doesNotMatch(
    panelSource,
    />Setup missing<|>Shared identity<|>Split data<|>Invalid identity</,
  );
