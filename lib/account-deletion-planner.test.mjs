  ...
  try {
    ...
    await client.batch(
      buildAccountDeletionGraphCleanupStatements({
        graph,
        deletedPurchaseOwner: "deleted:unused",
      }),
      "write",
    );
...
test("graph cleanup leaves no product remnants on aliases or canonical", async () => {
  ...
  try {
    ...
    await client.batch(
      buildAccountDeletionGraphCleanupStatements({
        graph,
        deletedPurchaseOwner: "deleted:test-owner",
      }),
      "write",
    );
    for (const table of [
    ...
    assert.equal(purchase.rows[0].anon_user_id, "deleted:test-owner");
