...
test("account deletion redacts and retains linked purchase financial evidence", async () => {
  const client = await createFixture(`
    INSERT INTO credit_purchase_transactions (
      row_id, provider, provider_transaction_id, anon_user_id,
  ...
  try {
    ...
    const pseudonym = createAccountDeletionPseudonym({
      ...
      identityNodes: ["user-delete"],
      secret: "purchase-migration-test-secret-long-enough",
    });
    await client.batch(
      buildPurchaseFinancialRetentionStatements({
        identityNodes: ["user-delete"],
        pseudonymousOwner: pseudonym,
      }),
      "write",
    );
    ...
    const ledgerOwner = (
      await client.execute(
        `SELECT anon_user_id FROM credit_ledger_entries
         WHERE entry_id = 'ledger-delete'`,
      )
    ).rows[0]?.anon_user_id;
    assert.equal(ledgerOwner, pseudonym);
    const untouched = (
      await client.execute(
